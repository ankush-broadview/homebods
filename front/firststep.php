...
if (isset($_POST['add_to_cart'])){
	...
	if ($_POST['s_m_qty'] == - 1){
		...
		if (isset($_SESSION['ct_cart']['method'])){
			...
			if ($idss != - 1){
				...
				if (count((array)$_SESSION['ct_cart']["method"]) == 0){
				...
				for($iq=0;$iq<sizeof((array)$_SESSION['ct_cart']['method']);$iq++){
	...
	else{ /* for first time add item into cart when cart is empty */
	
	
		if (isset($_SESSION['ct_cart']["method"]) && count((array)$_SESSION['ct_cart']["method"]) == 0){
			...
			if($_POST['staff_id']!=''){
				...
				$staff_detail = $objadmin->get_staff_details();
				
				if($_POST['service_id']==9 || $_POST['service_id']==10){
					if($_POST['method_name']=='Single Customer'){
						$total = $staff_detail[0];
					}
					if($_POST['method_name']=='Up To 3 Customer'){
						$total = $staff_detail[1];
					}
					if($_POST['method_name']=='Up To 5 Customer'){
						$total = $staff_detail[2];
					}
											...
											<td>
												<div style="padding: 25px 30px;background: #fff;float: left;width: 90%;display: block;">
													<div style="border-bottom: 1px solid #e6e6e6;float: left;width: 100%;display: block;">
														...
														<p style="color: #606060;font-size: 15px;margin: 10px 0px 15px;">Forgot your password - <a href="' . SITE_URL . 'admin/forgot-password.php?code=' . $ency_code . '" >Reset it here</a></p>
													</div>
													<div style="padding: 15px 0px;float: left;width: 100%;">
														<h5 style="color: #606060;font-size: 13px;margin: 10px 0px px;">Regards,</h5>
														<h6 style="color: #606060;font-size: 14px;font-weight: 600;margin: 10px 0px 15px;">' . $company_name . '</h6>
													</div>
												</div>
											</td>
										</tr>
