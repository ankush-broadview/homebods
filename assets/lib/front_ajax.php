...
// include(dirname(dirname(dirname(__FILE__))).'/User.php');
...
<?php 
    ...
    if(!$existing_login){
        ...
        $_SESSION['ct_username'] = $existing_login[21];
        $_SESSION['ct_name'] = $existing_login[3].' '.$existing_login[4];
        $_SESSION['fullname'] = $existing_login[3].' '.$existing_login[4];
        $_SESSION['user_uuid'] = $existing_login[23];
        $_SESSION['user_img'] = $existing_login[16];
...
elseif(isset($_POST['operationgetmethods']))
{
    ...
    $_SESSION['rate_type'] = 'Single Customer';
...
}elseif(isset($_POST['staff_select_according_postal'])){
	@ob_clean();
	ob_start();
	/* $postal_code_list =$settings->get_option_postal(); */
	
	$postal_code = $_POST['postal_code'];
	$objadmin->postal_code = $postal_code;
    $postal_provider_list =$objadmin->get_postal_acc_provider();
	
	
	unset($_SESSION['provider_sec']);
	$_SESSION['provider_sec'] = "";
	while($row = mysqli_fetch_array($postal_provider_list)){
...
<?php 
    ...
    $_SESSION['rate_type'] = $_POST['rate_type'];
    $objservice_method_unit->services_id = $_POST['service_id'];
    $objservice_method_unit->methods_id = $_POST['method_id'];
    ...
    while($unt_values_3 = mysqli_fetch_array($unt_valuess_3)){
        ...
        <div class="ct-bedrooms ct-btn-group fl ct-md-12 mt-5 mb-15 np" style="display: none;">
...
if(isset($_POST['get_updated_staff_detail'])){
    $staff_list = $_SESSION['provider_sec'];
    ...
    foreach($get_staff as $value){
        if($value!=""){ 
            ...
            if ($postal_code_staff_detail['online_offered'] == 'Y') {
                $zoom_link = 'Yes. I will provide an online video link upon confirmation of booking.';
            }else{
                $zoom_link = 'No';
            }
...
if (isset($_POST['getUsers']) ) {
    $get_all_user = array();
    $existing_login = $user->readall();
    while ($row = mysqli_fetch_array($existing_login)) {
        array_push($get_all_user,$row);
    }
    echo json_encode(array("status" => 200,"message" => array("users" => $get_all_user)));
    exit();
}
// if (isset($_POST['connectUser']) ) {
//     $user_1_uuid = $_POST['user_1'];
//     $user_2_uuid = $_POST['user_2'];

//     if (empty($user_1_uuid) || empty( $user_2_uuid)) {
//         return  array('status' => 303, 'message'=> 'Invalid details');
//     }

//     $result = $user->createChatRecord($user_1_uuid, $user_2_uuid);
...
//     if(count($row) > 1){
//         $ar['chat_uuid'] = $row['uuid'];
//     }else{
//         $chat_uuid = $user->getUuid();
//         $begin_chat_stmt = $user->insertChatRecord($chat_uuid,$user_1_uuid,$user_2_uuid);
